...
function NarrationScreen({ session }: { session: SessionInfo }) {
  ...
  useEffect(() => {
    ...
    const kicks: Record<string, string> = {
      ...
      'blackthorn-clan':
        "[DM]: Open Scenario 1 — from Tarric's perspective. Set the scene at the edge of the woodline at dawn. The full opening must establish all of the following as seen fact, not assumption: (1) why Tarric is here — Wynn was taken for ransom in the night, tracked through the woods with Briar for 2.5 hours, three sets of prints, one deep; (2) the mill across the clearing, old stonework, roof slumped, a lookout on the roof with a shortbow who hasn't seen them; (3) the stream running unusually low along the north bank — that is the approach; (4) that Tarric crept to the first window and saw Wynn — she is on a cot directly below it, hands chained to the head of the cot, feet tied to the foot, a gag in her mouth, still in sleeping clothes; a guard sits in the chair near the interior door, nodding off; Wynn looks up and sees Tarric, who puts a finger to his lips. This is not assumed — Tarric saw it. Emit state_changes to flip Wynn's discovered to true. End with an actions_required of type confirm asking Wynn's player to place Tarric and Briar on the map using the Party sidebar before the story continues.",
      'random-encounter':
